    ...
    const handleAuth = async (e) => {
        ...
        try {
            const response = await fetch(`${API_URL}${endpoint}`, {
                ...
                body: JSON.stringify({ username: safeUser, password: authSecret })
            });
    ...
    return (
        <div className="min-h-screen bg-[#06101e] flex flex-col items-center justify-center p-4 text-white relative overflow-hidden">
           ...
           <div className="bg-[#0b1829]/80 p-8 rounded-3xl border border-cyan-500/20 backdrop-blur-xl z-10 w-full max-w-md shadow-[0_0_40px_rgba(6,182,212,0.1)] text-center relative">
              ...
              <p className="text-slate-400 text-sm mb-8">Masuk ke Server MySQL VPS</p>
              
              <form onSubmit={handleAuth} className="space-y-4">
                 <input 
                    type="text" value={authInput} onChange={e => setAuthInput(e.target.value.replace(/\s/g, ''))} 
                    autoCapitalize="none" autoComplete="off" autoCorrect="off" spellCheck="false"
                    placeholder="Username (ID)" 
                    className="w-full bg-[#06101e] border border-cyan-500/20 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-cyan-400 transition-colors shadow-inner lowercase" required 
                 />
                 <input 
                    type="password" value={authSecret} onChange={e => setAuthSecret(e.target.value)} 
                    placeholder="Password (Min. 6 Karakter)" 
                    className="w-full bg-[#06101e] border border-cyan-500/20 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-cyan-400 transition-colors shadow-inner" required autoCapitalize="none" autoCorrect="off"
                 />
                 
                 <button type="submit" disabled={isLoading} className={`w-full py-3.5 rounded-xl font-bold text-white shadow-lg transition-all active:scale-95 flex items-center justify-center gap-2 ${isRegistering ? 'bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 shadow-purple-500/30' : 'bg-gradient-to-r from-cyan-600 to-sky-600 hover:from-cyan-500 hover:to-sky-500 shadow-cyan-500/30'} ${isLoading ? 'opacity-50 cursor-not-allowed' : ''}`}>
                    {isLoading ? <RefreshCw className="animate-spin" size={20} /> : (isRegistering ? 'Buat Akun Baru' : 'Masuk ke Server')}
                 </button>
              </form>
...
// ==========================================
// 2. KOMPONEN: GAME ENGINE UTAMA
// ==========================================
function GameEngine({ session, onLogout, showToast }) {
  ...
  const handleAdminDirectSend = (e) => {
      ...
      sendApiAction('sendMail', {
         id: Date.now().toString(), receiver_uid: targetPlayer.id, title: "Hadiah dari Admin",
         body: `Admin Server mengirimkan hadiah. ${adminSendMessage}`, type: 'admin_gift',
         has_attachment: true, attachment_type: adminSendType, attachment_amount: Number(adminSendAmount), timestamp: Date.now()
      });
      showToast(`Berhasil mengirim hadiah ke ${targetPlayer.username}!`, "success");
      setAdminSendAmount(''); setAdminSendMessage('');
  };

  const handleAdminSetRole = (e) => {
      e.preventDefault();
      if (!adminRoleTarget || !adminRoleType) return showToast("Lengkapi form pangkat!", "error");
      sendApiAction('adminAction', { type: 'setRole', targetId: adminRoleTarget, role: adminRoleType });
      showToast(`Pangkat diubah menjadi ${adminRoleType.toUpperCase()}`, 'success');
      setAdminRoleTarget('');
  };

  const handleAdminWipeBalance = (e) => {
      e.preventDefault();
      if (!wipeTarget) return showToast("Pilih player yang ingin di-wipe!", "error");
      sendApiAction('adminAction', { type: 'wipeMoney', targetId: wipeTarget });
      showToast(`Saldo target berhasil di-Wipe (Rp 0)`, 'success');
      setWipeTarget('');
  };

  const adminToggleBan = (targetUid, currentStatus) => {
      sendApiAction('adminAction', { type: 'toggleBan', targetId: targetUid, status: !currentStatus });
      showToast(`Player ${!currentStatus ? 'di-BAN' : 'di-UNBAN'}`, 'success');
  };

  const adminDeleteReport = (reportId) => {
      sendApiAction('deleteReport', { reportId });
      showToast("Laporan dihapus", "success");
  };

  const adminForceCollectPinjol = (targetUid) => {
      sendApiAction('adminAction', { type: 'wipeMoney', targetId: targetUid });
      showToast('Hutang ditagih paksa dan uang di-wipe!', 'success');
  };
  ...
  const serveFB = (slotKey) => {
      ...
      if (rental && rental.fbRequest) {
          ...
          setActiveRentals(prev => {
              const next = {...prev};
              if(next[slotKey]) { next[slotKey] = {...next[slotKey]}; delete next[slotKey].fbRequest; }
              return next;
          });
      }
  };
  ...
  const fixPS = (slotKey) => {
     if (activeRentals[slotKey]) {
        ...
        setActiveRentals(prev => {
            const next = {...prev};
            if (next[slotKey]) { next[slotKey] = {...next[slotKey], isBroken: false}; delete next[slotKey].dialog; }
            return next;
        });
     }
  };
  ...
  // PASTIKAN FUNGSI INI ADA TEPAT DI BAWAH uninstallPS 
  // DAN MASIH DI DALAM KOMPONEN UTAMA (Sebelum tulisan RENDER SCREENS)
  ...
  if (playerProfile?.isBanned) {
      return (
          <div className="min-h-screen bg-black text-white flex flex-col items-center justify-center p-4 text-center relative overflow-y-auto">
              <button onClick={onLogout} className="absolute top-6 right-6 flex items-center gap-2 bg-rose-500/20 text-rose-400 px-4 py-2 rounded-xl border border-rose-500/30 hover:bg-rose-500 hover:text-white transition-all">
    ...
    <div className="h-screen flex flex-col bg-[#020617] text-slate-200 font-sans overflow-hidden">
      ...
      <div className="w-full flex flex-col shrink-0 z-50">
        {sysBroadcastMsg && (
           <div className="w-full bg-gradient-to-r from-amber-600 via-yellow-500 to-amber-600 text-amber-950 py-1.5 px-4 text-center text-[11px] md:text-xs font-black uppercase tracking-widest shadow-[0_0_20px_rgba(245,158,11,0.5)] animate-bounce flex justify-center items-center gap-2">
              <Crown size={16} /> [PENCAPAIAN] {String(sysBroadcastMsg)} <Crown size={16} />
           </div>
        )}
        ...
        {globalConfig.broadcast && (
           <div className="w-full bg-gradient-to-r from-sky-600 via-cyan-500 to-sky-600 text-white py-1.5 px-4 text-center text-[11px] md:text-xs font-black uppercase tracking-widest shadow-[0_0_15px_rgba(6,182,212,0.5)] animate-pulse flex justify-center items-center gap-2">
              <Megaphone size={14} /> {String(globalConfig.broadcast)} <Megaphone size={14} />
           </div>
        )}
      </div>
      ...
      <header className="bg-[#0b1829]/90 backdrop-blur-xl border-b border-cyan-900/30 p-3 px-4 md:px-6 flex justify-between items-center z-20 shrink-0 shadow-lg">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
               ...
               <span className={`flex items-center gap-1 text-[9px] px-2 py-0.5 rounded uppercase font-bold ${playerProfile.role==='admin'?'bg-indigo-500/20 text-indigo-400 border border-indigo-500/30' : playerProfile.role==='ceo'?'bg-red-500/20 text-red-400 border border-red-500/30' : playerProfile.role==='direktur'?'bg-cyan-500/20 text-cyan-300 border border-cyan-500/30' : playerProfile.role==='manajer'?'bg-yellow-500/20 text-yellow-400 border border-yellow-500/30' : playerProfile.role==='moderator'?'bg-amber-500/20 text-amber-400 border border-amber-500/30' : 'bg-slate-500/20 text-slate-400 border border-slate-500/30'}`}>
                  {playerProfile.role === 'admin' && <BadgeCheck size={12} />}
                  {playerProfile.role}
               </span>
            </div>
        ...
        <div className="flex gap-2 md:gap-3 items-center">
          {['manajer', 'direktur', 'ceo'].includes(playerProfile.role) && (playerProfile.last_daily_claim || 0) < new Date().setHours(0,0,0,0) && (
             <button onClick={handleClaimDaily} className="bg-gradient-to-r from-yellow-500 to-amber-600 hover:scale-105 text-amber-950 font-black px-3 py-1.5 rounded-full text-xs flex items-center gap-1 shadow-lg animate-pulse transition-transform">
          ...
          <button onClick={onLogout} className="bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/30 p-2 rounded-full text-rose-400 transition-colors ml-1" title="Log Out">
            <LogOut size={16} />
          </button>
        </div>
      </header>
                  ...
                  {(!playerProfile.mail || playerProfile.mail.length === 0) ? (
                     ...
                     [...playerProfile.mail].reverse().map(m => (
                        <div key={m.id} className={`bg-white/5 p-4 rounded-xl border ${m.title.includes('PENAGIHAN') ? 'border-rose-500/50 bg-rose-950/20' : m.type === 'admin_gift' ? 'border-amber-500/30 shadow-[0_0_15px_rgba(245,158,11,0.1)]' : 'border-white/10 hover:border-cyan-500/30'} transition-colors`}>
                           ...
                           <p className={`text-sm leading-relaxed whitespace-pre-line ${m.title.includes('PENAGIHAN') ? 'text-rose-200' : 'text-slate-300'}`}>{String(m.body)}</p>
                               ...
                               <div className="mt-4 pt-3 border-t border-white/10">
                                   {!m.is_claimed ? (
                                       <button onClick={() => handleClaimMail(m.id)} className="bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white text-xs font-bold px-4 py-2.5 rounded-lg shadow-lg transition-transform active:scale-95 flex items-center gap-2">
                                           <Sparkles size={14}/> Klaim {m.attachment_type === 'money' ? `Rp ${m.attachment_amount.toLocaleString()}` : `${m.attachment_amount} Unit ${PS_DATA[m.attachment_type]?.name || 'Item'}`}
                                       </button>
                                   ) : (
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <aside className="w-[70px] sm:w-[85px] md:w-[220px] bg-[#0b1829]/60 backdrop-blur-md border-r border-cyan-900/30 p-2 md:p-4 shrink-0 flex flex-col gap-2 z-10 overflow-y-auto custom-scrollbar">
          ...
          <NavButton id="berita" icon={Newspaper} label="Kabar Rental" alert={globalConfig.dailyNews !== undefined && globalConfig.dailyNews !== ''} />
               ...
               <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
                {Object.entries(inventory).map(([key, count]) => (
                  <div key={key} className="bg-[#0b1829]/60 p-6 rounded-3xl border border-white/10 text-center backdrop-blur-xl shadow-xl">
                    <div className="text-sm font-bold text-white mb-2 tracking-wide">{PS_DATA[key]?.name || 'Item'}</div>
                    ...
                    <button onClick={() => sellPS(key)} disabled={count === 0} className={`w-full py-3 rounded-xl text-sm font-bold transition-all shadow-lg ${count > 0 ? 'bg-rose-600 hover:bg-rose-500 text-white shadow-rose-900/50' : 'bg-black/30 text-slate-600 border border-white/5'}`}>Jual ({formatRp((PS_DATA[key]?.price || 0) * 0.7)})</button>
                  </div>
                ))}
              </div>
            </div>
                   ...
                   <div>
                      ...
                      <select value={tradeTarget} onChange={e => setTradeTarget(e.target.value)} className="w-full bg-[#06101e] border border-cyan-900/50 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-cyan-400">
                         ...
                         {allPlayers.filter(p => p.id !== playerProfile.id).map(p => (
                            <option key={p.id} value={p.id}>{p.username || 'Unknown'} ({p.role})</option>
                         ))}
                      </select>
                   </div>
                      ...
                      return (
                        <div key={role.key} className="bg-[#06101e]/80 border border-white/5 rounded-3xl p-5 flex flex-col justify-between h-[180px] shadow-lg">
                           <div className="flex items-center gap-3 mb-4 border-b border-white/5 pb-3">
                             <div className={`p-2 rounded-xl ${role.bg}`}><role.icon size={20} className={role.text} /></div>
                             <span className="text-sm font-bold text-white">{role.title}</span>
                           </div>
                           <div className="flex-1 flex flex-col justify-end">
                             {emp ? (
                               <div className={`${role.cardBg} ${role.border} p-4 rounded-2xl flex justify-between items-center shadow-inner`}>
                                 <div>
                                   <div className="text-sm font-bold text-white mb-0.5">{emp.name}</div>
                                   <div className={`text-[10px] font-bold ${role.text}`}>Rp {emp.salary.toLocaleString()}/hr</div>
                                 </div>
                               ...
                               <div className="bg-[#0b1829] border border-white/10 p-4 rounded-2xl flex justify-between items-center shadow-lg">
                                 ...
                                 <button onClick={() => hireEmployee(role.key)} className={`${role.btn} px-5 py-2.5 rounded-xl text-xs font-bold text-white shadow-lg`}>Rekrut</button>
                               </div>
                             )}
                           </div>
                        </div>
                     ...
                     <div className="bg-black/50 p-5 rounded-2xl border border-emerald-500/30 shadow-inner">
                        ...
                        <form onSubmit={handleAdminDirectSend} className="space-y-3">
                            <select value={adminSendTarget} onChange={e => setAdminSendTarget(e.target.value)} className="w-full bg-[#06101e] border border-white/10 rounded-lg px-3 py-2 text-xs text-white">
                               ...
                               {allPlayers.map(p => <option key={p.id} value={p.id}>{p.username || 'Unknown'}</option>)}
                            </select>
                            <div className="flex gap-2">
                           ...
                           <tbody>
                              {allPlayers.filter(p => (p.username || '').toLowerCase().includes((playerSearch || '').toLowerCase()) || p.id.includes(playerSearch)).map(p => (
                                 <tr key={p.id} className="border-b border-white/5 last:border-0 hover:bg-white/5 transition-colors">
                                    <td className="p-3">
                                       <div className={`font-bold text-sm flex items-center gap-1 ${p.is_banned ? 'text-rose-500 line-through' : 'text-cyan-300'}`}>
                                          {p.username || 'Unknown'} 
                                    ...
                                    <td className="p-3">
                                       {p.is_banned ? <span className="bg-rose-500/20 text-rose-400 px-2 py-0.5 rounded text-[9px] font-black uppercase shadow-[0_0_5px_rgba(225,29,72,0.3)]">BANNED</span> : <span className="bg-emerald-500/20 text-emerald-400 px-2 py-0.5 rounded text-[9px] font-black uppercase">Aktif</span>}
                                    </td>
                                    <td className="p-3 flex flex-wrap gap-2 items-center">
                                       {!ADMIN_ACCOUNTS.hasOwnProperty((p.username || '').toLowerCase()) && (
                                          <button onClick={() => adminToggleBan(p.id, p.is_banned)} className={`${p.is_banned ? 'bg-emerald-600 hover:bg-emerald-500' : 'bg-slate-600 hover:bg-slate-500'} text-white px-3 py-1.5 rounded-lg text-[10px] font-bold transition-all shadow-md`}>{p.is_banned ? 'UNBAN' : 'BAN'}</button>
                                       )}
                                       {p.debt > 0 && (
...
// ==========================================
// 3. KOMPONEN: APP ROOT (WRAPPER & TOAST)
// ==========================================
export default function App() {
   ...
   // Cek Auto-Login (Jika tidak mau login ulang terus-menerus)
   useEffect(() => {
       const token = localStorage.getItem('juragan_token');
       const userStr = localStorage.getItem('juragan_user');
       if (token && userStr) {
           try {
               setSession({ token, user: JSON.parse(userStr) });
           } catch(e) {
               localStorage.removeItem('juragan_user');
           }
       }
   }, []);

   const handleLoginSuccess = (user, token) => {
       localStorage.setItem('juragan_token', token);
       localStorage.setItem('juragan_user', JSON.stringify(user));
       setSession({ user, token });
   };

   const handleLogout = () => {
       localStorage.removeItem('juragan_token');
       localStorage.removeItem('juragan_user');
       setSession(null);
   };
   ...
   return (
       <>
          ...
          {!session ? (
              ...
              <GameEngine session={session} onLogout={handleLogout} showToast={showToast} />
          )}
