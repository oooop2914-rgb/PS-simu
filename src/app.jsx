...
export default function App() {
  ...
  const handleRegisterLogin = async () => {
    ...
    try {
      const profileRef = doc(db, 'artifacts', appId, 'public', 'data', 'players', user.uid);
      let role = 'user';
      if (authInput === 'ps' && authSecret === 'ps12') role = 'admin';
      else if (authSecret === 'admin123') role = 'admin';
...
Username: ${playerProfile.username}`;
  ...
  const adminClearBroadcast = () => adminSetGlobal('broadcast', '');
  
  // ADMIN ADD SALDO MENU (MENGGUNAKAN MAIL)
  const handleAdminSendGift = async () => {
      if (!adminGiftTarget) return showToast("Pilih pemain tujuan!", "error");
      const amt = N    marketing: { id: 4, name: 'Dita', role: 'marketing', salary: 80000, desc: 'Banyak Pelanggan' },
    kasir: { id: 5, name: 'Adel', role: 'kasir', salary: 45000, desc: 'Bonus Income Rental +10%' },
    vip_manager: { id: 6, name: 'Kalag', role: 'vip_manager', salary: 150000, desc: 'Auto Usir Perokok VIP' }
  });
