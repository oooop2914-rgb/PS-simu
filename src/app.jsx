...
export default function App() {
  ...
  const [authErrorDisplay, setAuthErrorDisplay] = useState(null);
  ...
  const handleAuth = async (e) => {
    ...
    try {
      ...
      if (isRegistering) {
          ...
          showToast(`Be  const [activeTab, setActiveTab] = useState('rental');
  ...
  // ADMIN STATE
  const [broadcastInput, setBroadcastInput] = useState('');
  const [newsInput, setNewsInput] = useState(''); // BERITA HARIAN
  const [adminSendTarget, setAdminSendTarget] = useState('');
  const [adminSendType, setAdminSendType] = useState('money');
  const [adminSendAmount, setAdminSendAmount] = useState('');
  const [adminSendMessage, setAdminSendMessage] = useState(''); 
  ...
  // Fungsi Trigger Achievement Broadcast
  const triggerSystemBroadcast = async (msg) => {
      try {
          const configRef = doc(db, 'artifacts', appId, 'public', 'data', 'globalConfig', 'settings');
          await updateDoc(configRef, { sysBroadcast: { message: msg, time: Date.now() } });
  ...
  // Pengecekan Achievement Kekayaan
  useEffect(() => {
      if (!playerProfile || playerProfile.role === 'admin' || !user || !db) return;
      ...
      checkAchievements();
  }, [money, playerProfile, user]);

  // Realtime Sync Saldo & Inventory (Routine Save)
  useEffect(() => {
     if (!user || !playerProfile || gameOverRef.current) return;
     const interval = setInterval(() => {
        const profileRef = doc(db, 'artifacts', appId, 'public', 'data', 'players', user.uid);
        updateDoc(profileRef, { 
            money: moneyRef.current, 
     ...
     return () => clearInterval(interval);
  }, [user, playerProfile, inventory]);


  // --- SECURITY & LOGIN / REGISTER (UID BINDING) ---
  const handleRegisterLogin = async () => {
    const inputUser = authInput.trim();
    const inputPw = authSecret.trim();

    if (!inputUser) return showToast("Username tidak boleh kosong!", "error");
    if (!inputPw) return showToast("Password tidak boleh kosong!", "error");
    if (inputUser.length > 12) return showToast("Username maks 12 karakter!", "error");
    if (inputPw.length < 4) return showToast("Password minimal 4 karakter!", "error");
    
    try {
      const myExistingAccount = allPlayers.find(p => p.id === user.uid);
      const inputUserL = inputUser.toLowerCase();

      if (myExistingAccount) {
          if (myExistingAccount.username.toLowerCase() !== inputUserL) {
              return showToast(`Gagal! Perangkat ini telah terikat dengan akun: ${myExistingAccount.username}`, "error");
          }
          if (myExistingAccount.password !== inputPw) {
              return showToast("Akses Ditolak! Password Salah.", "error");
          }

          setPlayerProfile(myExistingAccount);
          setMoney(myExistingAccount.money || 1800000);
          setReputation(myExistingAccount.reputation !== undefined ? myExistingAccount.reputation : 100);
          if (myExistingAccount.inventory) setInventory(myExistingAccount.inventory);
          if (myExistingAccount.shopName) setShopName(myExistingAccount.shopName);
          if (myExistingAccount.fbUnlocked) setFbUnlocked(true);
          if (myExistingAccount.fanUnlocked) setFanUnlocked(true);
          if (myExistingAccount.acUnlocked) setAcUnlocked(true);
          lastTxIdRef.current = myExistingAccount.txId || 0;
          
          showToast(`Selamat datang kembali, ${myExistingAccount.username}!`, "success");
          return;
      }

      const usernameTaken = allPlayers.find(p => p.username.toLowerCase() === inputUserL);
      if (usernameTaken) {
          return showToast("Nickname sudah diklaim oleh player/perangkat lain! Pilih nama lain.", "error");
      }

      let role = 'user';
      if (ADMIN_ACCOUNTS[inputUserL]) {
          if (inputPw !== ADMIN_ACCOUNTS[inputUserL]) {
              return showToast("Akses Admin Ditolak! Password Salah.", "error");
          }
          role = 'admin';
      }
      ...
      setMoney(1800000);
      setReputation(100);
      showToast(`Berhasil mendaftar dan mem-bind akun ke perangkat ini!`, "success");

    } catch (e) {
      showToast("Gagal memproses data otentikasi.", "error");
    }
  };
  ...
  const handleLogout = () => {
      ...
      setAuthSecret('');
      setActiveTab('rental');
      showToast("Berhasil Keluar. Sesi perangkat diamankan.", "success");
  };
  ...
  const handleResetPassword = async () => {
      if (captchaInput.toUpperCase() !== captchaCode) return showToast("Kode keamanan salah!", "error");
      if (newPasswordInput.length < 4) return showToast("Password baru minimal 4 karakter!", "error");

      const myExistingAccount = allPlayers.find(p => p.id === user?.uid);
      if (!myExistingAccount) return showToast("Tidak ada akun yang terikat di perangkat ini.", "error");

      if (ADMIN_ACCOUNTS[myExistingAccount.username.toLowerCase()]) {
          return showToast("Akses Ditolak! Akun Admin tidak dapat direset dengan fitur ini.", "error");
      }

      try {
          const profileRef = doc(db, 'artifacts', appId, 'public', 'data', 'players', user.uid);
          await updateDoc(profileRef, { password: newPasswordInput });
          showToast("Password berhasil direset! Silakan login kembali.", "success");
  ...
  const handleTrade = async (e) => {
     ...
     if (targetPlayer.id === user.uid) return showToast("Tidak bisa trade ke diri sendiri!", "error");

     const amt = Number(tradeAmount);
     try {
        const senderRef = doc(db, 'artifacts', appId, 'public', 'data', 'players', user.uid);
  ...
  const adminChangeRole = async (targetUid, role) => {
      try {
         const profileRef = doc(db, 'artifacts', appId, 'public', 'data', 'players', targetUid);
         await updateDoc(profileRef, { role });
         showToast(`Role diubah menjadi ${role}`, 'success');
      } catch (e) { showToast("Gagal ubah role", 'error'); }
  };
  ...
  // FIX DISKON TOKO (Menghitung efektif diskon berdasarkan Global + Role)
  ...
  if (!playerProfile) {
     return (
        <div className="min-h-screen bg-[#06101e] flex flex-col items-center justify-center p-4 text-white relative overflow-hidden">
           ...
           <div className="bg-[#0b1829]/80 p-8 rounded-3xl border border-cyan-500/20 backdrop-blur-xl z-10 w-full max-w-md shadow-[0_0_40px_rgba(6,182,212,0.1)] text-center">
              ...
              {!isForgotPassword ? (
                 <>
                    <p className="text-slate-400 text-sm mb-8">Mulailah karir anda sebagai juragan. Masuk atau Daftar untuk lanjut bermain.</p>
                    <div className="space-y-4">
                       <input type="text" value={authInput} onChange={e => setAuthInput(e.target.value)} placeholder="Masukkan Nickname (ID)" className="w-full bg-[#06101e] border border-cyan-500/20 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-cyan-400 transition-colors shadow-inner" />
                       <input type="password" value={authSecret} onChange={e => setAuthSecret(e.target.value)} placeholder="Masukkan Password" className="w-full bg-[#06101e] border border-cyan-500/20 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-cyan-400 transition-colors shadow-inner" />
                       
                       <button onClick={handleRegisterLogin} className="w-full bg-gradient-to-r from-cyan-600 to-sky-600 hover:from-cyan-500 hover:to-sky-500 py-3.5 rounded-xl font-bold text-white shadow-[0_0_20px_rgba(6,182,212,0.3)] transition-all active:scale-95">Masuk ke Server</button>
                    </div>
                    <button onClick={() => { setIsForgotPassword(true); generateCaptcha(); }} className="text-xs text-cyan-400 hover:text-cyan-300 mt-5 underline decoration-cyan-900 underline-offset-4 transition-colors">Lupa Password?</button>
                    <p className="text-[10px] text-cyan-600/50 mt-5">*Sistem menggunakan Hardware Lock. 1 Perangkat = 1 Akun.</p>
                 </>
              ) : (
                 <>
                    <p className="text-slate-400 text-sm mb-6">Reset Password Akun Anda</p>
                    <div className="space-y-4">
                       <div className="bg-[#06101e] p-4 rounded-xl border border-cyan-900/50 flex flex-col items-center justify-center gap-2 shadow-inner">
                           <span className="text-xs text-slate-400">Ketik kode keamanan di bawah ini:</span>
                           <span className="text-3xl font-black text-emerald-400 tracking-widest drop-shadow-[0_0_10px_rgba(16,185,129,0.5)] select-none pointer-events-none">{captchaCode}</span>
                       </div>
                       <input type="text" value={captchaInput} onChange={e => setCaptchaInput(e.target.value)} placeholder="Masukkan Kode Keamanan" className="w-full bg-[#06101e] border border-emerald-500/30 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-emerald-400 transition-colors shadow-inner uppercase font-bold text-center" />
                       <input type="password" value={newPasswordInput} onChange={e => setNewPasswordInput(e.target.value)} placeholder="Password Baru (Min. 4 Karakter)" className="w-full bg-[#06101e] border border-cyan-500/20 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-cyan-400 transition-colors shadow-inner" />
                       
                       <div className="flex gap-3 pt-2">
                           <button onClick={() => { setIsForgotPassword(false); setCaptchaInput(''); setNewPasswordInput(''); }} className="flex-1 bg-slate-800 hover:bg-slate-700 py-3.5 rounded-xl font-bold text-white transition-all border border-slate-600">Batal</button>
                           <button onClick={handleResetPassword} className="flex-1 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 py-3.5 rounded-xl font-bold text-white shadow-[0_0_20px_rgba(16,185,129,0.3)] transition-all active:scale-95">Simpan</button>
                       </div>
                    </div>
                 </>
              )}
                ...
                <form onSubmit={handleTrade} className="space-y-5">
                   <div>
                      ...
                      <select value={tradeTarget} onChange={e => setTradeTarget(e.target.value)} className="w-full bg-[#06101e] border border-cyan-900/50 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-cyan-400">
                         ...
                         {allPlayers.filter(p => p.id !== user.uid).map(p => (
                ...
                <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div className="bg-[#0b1829]/80 p-6 rounded-3xl border-2 border-yellow-500/50 text-center shadow-[0_0_20px_rgba(234,179,8,0.2)] backdrop-blur-xl relative overflow-hidden group">
                           ...
                           <button onClick={() => window.open(`https://wa.me/6285122900934?text=Halo admin, saya mau beli Pangkat MANAJER. UID saya: ${user.uid}`)} className="w-full bg-yellow-600 hover:bg-yellow-500 py-3 rounded-xl text-xs font-black text-yellow-950 transition-all shadow-md uppercase tracking-wide">Beli via WA</button>
                        </div>
                        ...
                        <div className="bg-[#0b1829]/80 p-6 rounded-3xl border-2 border-cyan-400/50 text-center shadow-[0_0_30px_rgba(34,211,238,0.2)] backdrop-blur-xl relative overflow-hidden group transform md:-translate-y-4">
                           ...
                           <button onClick={() => window.open(`https://wa.me/6285122900934?text=Halo admin, saya mau beli Pangkat DIREKTUR. UID saya: ${user.uid}`)} className="w-full bg-cyan-500 hover:bg-cyan-400 py-3 rounded-xl text-xs font-black text-cyan-950 transition-all shadow-[0_0_15px_rgba(34,211,238,0.4)] uppercase tracking-wide">Beli via WA</button>
                        </div>
                        ...
                        <div className="bg-[#0b1829]/80 p-6 rounded-3xl border-2 border-red-600 text-center shadow-[0_0_40px_rgba(220,38,38,0.3)] backdrop-blur-xl relative overflow-hidden group">
                           ...
                           <button onClick={() => window.open(`https://wa.me/6285122900934?text=Halo admin, saya mau beli Pangkat CEO. UID saya: ${user.uid}`)} className="w-full bg-red-600 hover:bg-red-500 py-3 rounded-xl text-xs font-black text-white transition-all shadow-[0_0_20px_rgba(220,38,38,0.6)] uppercase tracking-wide">Beli via WA</button>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                       {[1, 2, 3, 4, 5].map((val) => (
                          <div key={val} className="bg-[#0b1829]/80 p-6 rounded-3xl border border-emerald-500/30 text-center shadow-[0_0_20px_rgba(16,185,129,0.1)] backdrop-blur-xl hover:border-emerald-500/60 transition-colors group">
                             ...
                             <div className="flex gap-2">
                                 <button onClick={() => window.open(`https://wa.me/6285122900934?text=Halo admin, saya mau order in-game balance ${val} Juta. UID saya: ${user.uid}`)} className="flex-1 bg-emerald-600 hover:bg-emerald-500 py-2 rounded-xl text-[10px] sm:text-xs font-bold text-white transition-all shadow-md">WA Admin 1</button>
                                 <button onClick={() => window.open(`https://wa.me/6285888233667?text=Halo admin, saya mau order in-game balance ${val} Juta. UID saya: ${user.uid}`)} className="flex-1 bg-teal-600 hover:bg-teal-500 py-2 rounded-xl text-[10px] sm:text-xs font-bold text-white transition-all shadow-md">WA Admin 2</button>
                             </div>
                          </div>
                       ))}
                    </div>
